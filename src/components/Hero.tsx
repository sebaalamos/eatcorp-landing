...
export function Hero() {
  ...
  return (
    <section className="pt-32 pb-20 px-4 bg-gradient-to-b from-primary-50 via-white to-white relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto text-center relative z-10">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 mb-6 rounded-full bg-primary-50 border border-primary-200">
          <span className="w-2 h-2 rounded-full bg-primary-500 animate-pulse"></span>
          <span className="text-sm font-medium text-primary-800">Hecho en Chile para restaurantes chilenos</span>
        </div>

        <h1 className="text-5xl md:text-7xl font-bold text-slate-900 mb-6 leading-tight tracking-tight">
          La plataforma todo-en-uno
          <br />
          para tu <span className="text-primary-600">restaurante</span>
        </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
          <button
            onClick={handleClick}
            className="bg-primary-600 hover:bg-primary-700 text-white px-8 py-4 rounded-lg font-semibold text-lg transition-all shadow-lg shadow-primary-600/20 hover:shadow-xl hover:shadow-primary-600/30"
          >
            Comienza gratis
          </button>
          <a
            href="#apps"
            className="border-2 border-slate-300 text-slate-700 hover:border-primary-600 hover:text-primary-700 px-8 py-4 rounded-lg font-semibold text-lg transition-colors"
          >
            Ver las apps
          </a>
        </div>
        ...
        <div className="relative max-w-5xl mx-auto">
          <div className="absolute inset-0 bg-gradient-to-tr from-primary-200/40 to-amber-100/40 rounded-2xl blur-2xl"></div>
          ...
          <div className={`absolute -top-4 -right-4 md:-right-8 z-20 transition-all duration-700 ${showToast ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-3 pointer-events-none'}`}>
            <div className="bg-white rounded-xl shadow-2xl border border-primary-200 px-4 py-3 flex items-center gap-3 max-w-xs">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
            <div className="bg-brand-900 border-b border-brand-700 px-4 py-3 flex items-center gap-2">
              <div className="flex gap-1.5">
                <div className="w-3 h-3 rounded-full bg-red-400"></div>
                <div className="w-3 h-3 rounded-full bg-yellow-400"></div>
                <div className="w-3 h-3 rounded-full bg-green-400"></div>
              </div>
              <div className="flex-1 mx-4 bg-brand-800 border border-brand-700 rounded px-3 py-1 text-xs text-slate-400">
                app.eatcorp.cl
              </div>
            </div>
    ...
    <div className="bg-white rounded-lg border border-slate-200 p-4 text-left">
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <div className={`flex items-center gap-1 mt-1.5 text-[10px] ${trendUp ? 'text-primary-700' : 'text-slate-500'}`}>
        ...
        <svg width={w} height={h} className="overflow-visible">
          <polyline
            ...
            className={trendUp ? 'text-primary-500' : 'text-slate-400'}
          />
        </svg>
      </div>
    </div>
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1 text-[10px] text-primary-700">
          <TrendingUp size={10} />
          <span>+18%</span>
        </div>
      </div>
      <div className="flex gap-2">
        ...
        <div className="flex-1">
          <div className="flex items-end gap-1 h-20 border-l border-b border-slate-200 pl-1 pb-0.5">
            {data.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                <div
                  className={`w-full bg-gradient-to-t from-primary-600 to-primary-400 rounded-t origin-bottom ${i === today ? 'ring-2 ring-primary-300 ring-offset-1' : ''}`}
          ...
          <div className="flex gap-1 mt-1">
            {data.map((d, i) => (
              <div key={i} className={`flex-1 text-center text-[9px] ${i === today ? 'text-primary-700 font-bold' : 'text-slate-400'}`}>
                {d.d}
              </div>
            ))}
          </div>
...
function TasksCard() {
  const tasks = [
    { label: 'Reponer aceite oliva', cat: 'Cocina', catColor: 'bg-primary-100 text-primary-700', who: 'M', whoBg: 'bg-primary-500' },
...
function ApprovalsCard() {
  const items = [
    ...
    { name: 'Carnes del Sur', amount: '$320K', urgency: 'Pronto pago -2%', urgColor: 'bg-primary-100 text-primary-700' },
